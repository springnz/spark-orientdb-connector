...
class ClassRDDFunctions[T](rdd: RDD[T]) extends Serializable with Logging {
  ...
  /*
	 ...
	 */
  @deprecated
  def old_saveToOrient(myClass: String)(implicit connector: OrientDBConnector = OrientDBConnector(rdd.sparkContext.getConf)): Unit = {

    rdd.foreachPartition { objs =>
      {
        val db = connector.databaseDocumentTx()
        val orientClass = connector.getSchema(db).getClass(myClass)
        while (objs.hasNext) {
          val obj = objs.next()
          val set = s"INSERT INTO $myClass " + getInsertString(obj)
          val insertCommand = new OCommandSQL(set)
          //            db.command(insertCommand).execute()
          connector.executeCommand(db, insertCommand)
        }
      }
    }
  }
